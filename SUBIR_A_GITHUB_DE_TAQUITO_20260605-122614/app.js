...
const defaultDeals = [
  ...
  },
  {
    ...
    oldPriceText: "US$ 32",
    badge: "Precio bajo",
    text: "Accesorio rápido para completar setup sin gastar demasiado.",
...
async function loadStaticCatalog() {
  ...
  if (!response.ok) throw new Error("No se pudo cargar el catálogo estático");
...
async function loadExchangeRate() {
  try {
    ...
    try {
      ...
      if (!response.ok) throw new Error("No se pudo cargar cotización estática");
      const data = await response.json();
...
async function loadAutomaticDeals({ refresh = false } = {}) {
  ...
  try {
    ...
    if (!response.ok) throw new Error("No se pudieron cargar ofertas automáticas");
    const data = await response.json();
...
function renderRateStatus() {
  ...
  const note = exchangeRate.fallback ? "modo respaldo" : "cotización actual";
...
function openProductDetail(product) {
  ...
  modalHistory.innerHTML = history.length
    ...
    : '<p class="muted">Todavía no hay historial suficiente para este producto.</p>';
  ...
  if (!visibleDeals.length) {
    dealsGrid.innerHTML = `
      <div class="empty-state">
        ...
        <span>${automaticDealsLoaded ? "La sección evita mostrar precios manuales desactualizados. Volverá a intentar al recargar." : "Estamos consultando tiendas conectadas para traer precios vigentes."}</span>
      </div>
    `;
    return;
  }
...
async function searchRealPrices() {
  ...
  try {
    ...
    state.message = offers
      ...
      : "No encontramos ofertas reales para esa búsqueda";
    ...
    try {
      ...
      state.store = "Todas";
      const offers = state.products.reduce((sum, product) => sum + product.offers.length, 0);
      state.sourceSummary = data.generatedAt ? `Catálogo actualizado: ${formatDateTime(data.generatedAt)}` : "Catálogo estático";
      state.sources = data.sources || [];
      state.cache = data.cache || null;
      state.lastGeneratedAt = data.generatedAt || "";
      state.message = data.emptyCatalog
        ? "Catálogo automático pendiente. Ejecuta la acción de GitHub para generar precios reales."
        : offers
        ? `${offers} ofertas del catálogo actualizado automáticamente`
        : "No encontramos ofertas en el catálogo actualizado";
    } catch {
      state.mode = "real";
      state.products = [];
      state.resultsVisible = true;
      state.message = "Catálogo automático no disponible en esta publicación.";
      state.sourceSummary = "Sube data/live-catalog.json y ejecuta la acción Actualizar datos estáticos en GitHub.";
      state.sources = [];
      state.cache = null;
      state.lastGeneratedAt = "";
    }
